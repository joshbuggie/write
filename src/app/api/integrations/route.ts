...
export const POST = handle(async (req) => {
  const { name, kind, folders, canCreate, launcher } = await readJson(req, isIntegrationRequest);
  const { integration, token } = await createIntegration({ name, kind, folders, canCreate, launcher });
  const body: IntegrationTokenResponse = { integration: toIntegrationView(integration), token };
  return json(body, 201);
});

/** Changes an integration's name, kind, folders, note creation or launcher. Its token keeps working. */
export const PATCH = handle(async (req) => {
  const { id, name, kind, folders, canCreate, launcher } = await readJson(req, isUpdateIntegrationRequest);
  const body: IntegrationResponse = {
    integration: toIntegrationView(await updateIntegration(id, { name, kind, folders, canCreate, launcher })),
  };
  return json(body);
});
